...
Two #gls("MT5")-base models were trained for grammar correction. From a naming convention at Ordbogen, the models are named four letter names in the native language. The first model in named Kári(Kari internally because the danes on the team don't know how to pronounce accents), and the second model is named Bára(Bara internally for the same reason). The models were trained on different datasets and using different corruption methods. Further details about the datasets and corruption methods can be found in each of their sections, @results_grammar__kari.sec and @results_grammar__bara.sec. The models were trained using the same training parameters. // TODO add training parameters
There are six error types have testsets, but to make the errors in a manner that won't just introduce uncontrolled noise, a dependency parser is needed. These error types are *Verb Tense Inconsistent*, *Pronoun - Antecendent agreement*, *Verb - Subject agreement*, *Case - Verb*, *Case  - Preposition* and *Gender*.
