...
For the sake of clarity, the metrics on @pos_morph_comp, will be explained: The "Tag" refers to the fine-grained #gls("POS") tag that is predicted by the #gls("POS") tagger. The #gls("POS") refers to the coarse-grained #gls("POS") tag, that is predicted by the morphologizer. The #gls("MORPH") refers to the morphological features that are predicted by the morphologizer.
#figure( 
...
While the results look good, after some manual inspection of some prediction, it is clear that the #gls("POS") tagger struggles with words like *skrivað* (written) if the context is "Bókin var *skrivað* í ár." (The book was written this year.), *skrivað* is an adjective, but if the context is "Eg havi *skrivað* bókina í ár." (I have written the book this year.), then it is a verb. The model struggles to disambiguate these two cases, and it is not the only case where this happens, but it is the most common case. This is likely a problem with the training data, where there is not enough examples of the different contexts, and the model is not able to learn the difference.
Just looking at the results, the spaCy model outperforms the Stanza model, when it comes to overall accuracy of the #gls("POS") tagger and morphologizer, but with the limited data and both training sets having major limitations its not clear how their performance generalizes to unseen data. On @morph_feats, a more detailed view of the performance of the morphologizer is shown, the table shows the precision, recall and $F_(1)$ score for each feature.
#figure(
  caption: flex-caption(
