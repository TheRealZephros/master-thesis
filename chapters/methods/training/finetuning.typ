...
The models were trained with the dual objective of both correcting errors in the input text and predicting the corresponding error types. This output format aligns with recent developments in explainable #gls("GEC") models, where corrections are annotated with specific categories for improved interpretability and evaluation. \
