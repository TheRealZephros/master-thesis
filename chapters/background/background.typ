...
The way #gls("TN") is counted is non-standard for the #gls("MT5") models, as it it only counted, when a whole sentence is correctly predicted to not contain any errors.
...
@guo2024 provide both empirical and theoretical evidence for the effectiveness of label smoothing as a regularization method.
...
The task was initially motivated by applications in second language learning and writing assistance @Leacock2014, but has since become relevant for broader #gls("NLP") tasks, such as preprocessing noisy text in user-generated content or learner corpora for use in machine translation, summarization, and other downstream models.
...
For this study , the focus is on a #gls("S2S") approach to #gls("GEC").

=== Sequence Tagging <background.seq_tag.sec>
Sequence tagging is a common approach in #gls("NLP") tasks, where the goal is to assign labels to each element in a sequence. This can be applied to various tasks, such as #gls("POS"), #gls("NER"), and #gls("GEC"). In the context of #gls("GEC"), sequence tagging involves predicting a sequence of edits that transform the input into a grammatically correct output. This approach allows for efficient processing and can leverage existing pre-trained models to improve performance on low-resource languages.
...
While this study adopts a #gls("S2S") approach, sequence tagging remains relevant as a framework for structuring training data. Inspired by tagging-based annotations, the data includes aligned edits with associated error types, which not only guide the #gls("S2S") model during training but also enable user-facing feedback by identifying the nature of each correction.

==== Part of Speech Tagging & Morphological Analysis <background.pos.sec>
#gls("POS") tagging and morphological tagging are foundational tasks in #gls("NLP"), providing structured linguistic annotations that support downstream applications such as syntactic parsing, #gls("NER"), and #gls("GEC"). For morphologically rich and ultra-low-resource languages like Faroese, accurate tagging is especially important, as key grammatical information is often expressed morphologically rather than through fixed word order .
#gls("POS") tagging assigns each token in a sentence a label that indicates its syntactic category, such as NOUN, VERB, or ADJ. These categories define the functional role of the word in a sentence and are typically drawn from a tagset such as the Universal #gls("POS") tagset used in the #gls("UD") project @Petrov2012 @Nivre2020. These standardized tags help ensure consistency across languages, enabling cross-linguistic comparison and model transfer.
...
The morphologizer component in spaCy learns from structured annotations and attempts to predict multiple features independently, although joint feature modeling is also possible.
